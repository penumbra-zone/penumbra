...
/// Test that we only fill up to the specified spill price.
/// TODO(erwan): stub, fleshing this out later.
async fn fill_route_with_dust() -> anyhow::Result<()> {
    let storage = TempStorage::new().await?.apply_default_genesis().await?;
    let mut state = Arc::new(StateDelta::new(storage.latest_snapshot()));
    let mut state_tx = state.try_begin_transaction().unwrap();
    /*
    ...
    */
    ...
    let buy_1 = Position::new(
        OsRng,
        pair_1.into_directed_trading_pair(),
        0u32,
        one * pair_1.end.unit_amount(),
    ...
    let buy_1 = Position::new(
        ...
        0u32,
        one * pair_2.end.unit_amount(),
        Amount::from(1u64) * pair_2.start.unit_amount(),
        Reserves {
            r1: Amount::zero(),
            r2: one,
        },
    ...
    let buy_1 = Position::new(
        ...
        Amount::from(4_000_000_000u64) * pair_3.start.unit_amount(),
        Reserves {
            r1: Amount::zero(),
            r2: one,
        },
    ...
    let desired_output = Amount::from(1u64);
