...
// export keys and genesis to files for testing
// TODO: this is kind of useful to have in a test utility crate
// pub fn export_key() {
//     // Write this node's priv_validator_key.json
//     let priv_validator_key_filepath = "/tmp/priv_validator_key.json";
//     tracing::debug!(priv_validator_key_filepath = %priv_validator_key_filepath, "writing validator private key");
//     let mut priv_validator_key_file = File::create(priv_validator_key_filepath)?;
//     // let priv_validator_key: PrivValidatorKey = sk_b.into();
//     let pub_key_a =
//         tendermint::PublicKey::from_raw_ed25519(vk_a.as_bytes()).expect("pub key present");
//     let pub_key_b =
//         tendermint::PublicKey::from_raw_ed25519(vk_b.as_bytes()).expect("pub key present");
//     let address_a = tendermint::account::Id::new(
//         <sha2::Sha256 as sha2::Digest>::digest(vk_a.as_bytes()).as_slice()[0..20]
//             .try_into()
//             .expect(""),
//     );
...
//         address: address_a,
//         pub_key: pub_key_a,
//         priv_key: tendermint::PrivateKey::Ed25519(
//             tendermint::crypto::ed25519::SigningKey::try_from(sk_a.clone().as_bytes())?,
//         ),
//     };
...
//         let spend_key_a = SpendKey::from(vkeys_a.validator_spend_key.clone());
//         let spend_key_b = SpendKey::from(vkeys_b.validator_spend_key.clone());
//         tendermint::PublicKey::from_raw_ed25519(&vkeys_a.validator_cons_pk.to_bytes())
...
//             identity_key: Some(IdentityKey(vkeys_b.validator_id_vk.into()).into()),
//             governance_key: Some(GovernanceKey(spend_key_b.spend_auth_key().into()).into()),
//             consensus_key: vkeys_b.validator_cons_pk.to_bytes(),
//             name: "test".to_string(),
//             website: "https://example.com".to_string(),
//             description: "test".to_string(),
//             enabled: true,
...
//             .validators
//             .push(validator_a.clone());
//         // TODO: actually let's only do one validator per chain for now
//         // since it's easier to validate against cometbft
//         // genesis_contents
//         //     .stake_content
//         //     .validators
//         //     .push(validator_b.clone());

//         // the two validators need some initial delegations
...
//         let identity_key_b: IdentityKey = IdentityKey(
//             spend_key_b
//                 .full_viewing_key()
//                 .spend_verification_key()
//                 .clone()
//                 .into(),
//         );
//         let delegation_id_b = DelegationToken::from(&identity_key_b).denom();
//         let ivk_b = spend_key_b.incoming_viewing_key();
//         // genesis_contents
//         //     .shielded_pool_content
//         //     .allocations
//         //     .push(Allocation {
//         //         address: ivk_b.payment_address(0u32.into()).0,
//         //         raw_amount: (25_000 * 10u128.pow(6)).into(),
//         //         raw_denom: delegation_id_b.to_string(),
//         //     });
...
//             // always empty in genesis json
//             app_hash: tendermint::AppHash::default(),
//             app_state: penumbra_app::genesis::AppState::Content(genesis_contents),
//             // Set empty validator set for Tendermint config, which falls back to reading
//             // validators from the AppState, via ResponseInitChain:
//             // https://docs.tendermint.com/v0.32/tendermint-core/using-tendermint.html
//             validators: vec![],
//         };
//         let genesis_file = "/tmp/genesis.json";
//         tracing::info!(?genesis_file, "writing genesis file to comet config");
//         let mut genesis_file = File::create(genesis_file)?;
