...
impl TryFrom<pb::SwapPlan> for SwapPlan {
    ...
    fn try_from(msg: pb::SwapPlan) -> Result<Self, Self::Error> {
        ...
        Ok(Self {
            fee_blinding: Fr::from_bytes(fee_blinding_bytes).expect("fee blinding malformed"),
            swap_plaintext: msg
                .swap_plaintext
                .ok_or_else(|| anyhow!("missing swap_plaintext"))?
                .try_into()
                .context("swap plaintext malformed")?,
            proof_blinding_r: Fq::from_bytes(proof_blinding_r_bytes).expect("expected deserialize"),
            proof_blinding_s: Fq::from_bytes(proof_blinding_s_bytes).expect("expected deserialize"),
        })
    }
}
