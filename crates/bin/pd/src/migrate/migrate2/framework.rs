...
pub trait Migration {
    ...
    async fn run(
        ...
        let (pre_upgrade_root_hash, pre_upgrade_height) = self.prepare(&pd_home, comet_home.as_ref()).await?;
        tracing::info!(
        ...
        let (post_upgrade_root_hash, post_upgrade_height) = self.migrate(&pd_home, comet_home.as_ref()).await?;
