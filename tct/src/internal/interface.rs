...
/// tree.
pub trait Focus<Hasher>:
    Height<Height = <Self::Complete as Height>::Height> + GetHash<Hasher>
{
    /// The [`Complete`] of this [`Active`].
    type Complete: Complete<Hasher, Focus = Self>;
...
pub trait Complete<Hasher>: Height + GetHash<Hasher> {
    /// The [`Focus`] of this [`Complete`].
    type Focus: Focus<Hasher, Complete = Self>;
}

/// The result of [`Active::insert`] when the [`Active`] is full.
pub struct Full<T: Active<Hasher>, Hasher> {
    /// The original hash or item that could not be inserted.
    pub item: Insert<T::Item, Hasher>,
    ...
    pub complete: Insert<T::Complete, Hasher>,
}
/// Witness an authentication path into a tree, or remove a witnessed item from one.
pub trait Witness<Hasher>: Height + Sized
where
    Self::Height: path::Path<Hasher>,
{
...
/// `Self` into a hash.
pub trait ForgetOwned<Hasher>: Height + Sized + GetHash<Hasher> {
