...
#[derive(Clone, Debug, Error)]
#[error("cache for `{}::{}` at height {}, index {} is incorrect: found {cached:?}, expected {recomputed:?}", .node.place, .node.kind, .node.height, .node.index)]
pub struct InvalidCachedHash {
    /// A description of the node at which the error occurred.
    pub node: Any,
