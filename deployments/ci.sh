...
cat <<EOM
Deploying release '$HELM_RELEASE' with:
  * $NFULLNODES fullnodes
  * $NVALS validators
  * '$PENUMBRA_VERSION' as version
EOM
...
# Here we duplicate a hard-coded validator identity for NVALS. The `pd testnet generate`
# command will accept a list of validators and generate necessary configs.
# For now, we just want the single 'Penumbra Labs CI 1' validator, which is why we
# default to NVALS=1.
...
# If NVALS=1, default to the 'Penumbra Labs CI 1' validator identity.
if [[ "$NVALS" -gt 1 ]] ; then
    validators_input_file_opt="--validators-input-file='${CONTAINERHOME}/vals.json'"
else
    validators_input_file_opt=""
fi
